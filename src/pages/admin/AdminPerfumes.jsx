...
const EMPTY_FORM = {
  ...
  volumeMl: "", scentFamily: "", concentration: "",
  isAlcoholFree: false, longevityHours: "",
...
export default function AdminPerfumes() {
  ...
  const fetchItems = async () => {
    ...
    } catch { showToast("Failed to load perfumes", "error"); }
  ...
  const openEdit = (item) => {
    ...
    setFormData({
      ...
      volumeMl: String(item.volumeMl ?? ""), scentFamily: item.scentFamily || "",
      concentration: item.concentration || "", isAlcoholFree: !!item.isAlcoholFree,
      ...
      ingredients: item.ingredients || "", altText: item.altText || "",
    });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        volumeMl: Number(formData.volumeMl) || 0,
    ...
    <div className="space-y-4 font-sans">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 px-1 pt-1 mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl sm:text-2xl font-bold tracking-tight text-gray-900">Perfumes & Attar</h2>
        ...
        <button onClick={openAdd} className="flex-1 sm:flex-none px-5 py-2.5 bg-black text-white rounded-xl font-bold text-xs hover:bg-gray-800 transition-all active:scale-95 h-10 flex items-center justify-center gap-2 shadow-lg shadow-gray-200">
          <Plus className="w-4 h-4" /><span>New Perfume / Attar</span>
        </button>
      </div>
      ...
      {!loading && filtered.length === 0 && (
        <div className="border rounded bg-white p-8 text-center">
          ...
          <p className="font-medium text-gray-900">{searchTerm ? "No items match your search" : "No perfumes yet"}</p>
          <p className="text-sm text-gray-500 mt-1">{searchTerm ? "Try adjusting your search" : "Add your first perfume or attar to get started."}</p>
          {!searchTerm && <button onClick={openAdd} className="mt-4 px-4 py-2 bg-black text-white rounded-xl flex items-center gap-2 mx-auto text-sm font-bold"><Plus className="w-4 h-4" /> Add Perfume</button>}
        </div>
      )}
            ...
            <div key={item.id} className={`bg-white rounded-2xl border-2 overflow-hidden shadow-sm hover:shadow-md transition-all group ${item.isActive === false ? "border-red-100 opacity-70" : "border-transparent"}`}>
              ...
              <div className="p-3 space-y-1">
                ...
                <div className="flex flex-wrap gap-1">
                  ...
                  {item.volumeMl > 0 && <span className="text-[9px] bg-gray-100 text-gray-600 px-1.5 py-0.5 rounded-full font-medium">{item.volumeMl}ml</span>}
                  {item.concentration && <span className="text-[9px] bg-gray-100 text-gray-600 px-1.5 py-0.5 rounded-full font-medium">{item.concentration}</span>}
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white w-full max-w-2xl rounded-3xl shadow-2xl max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 bg-white z-10 flex items-center justify-between p-6 border-b border-gray-100">
              <h3 className="text-lg font-bold text-gray-900">{editingId ? "Edit Perfume / Attar" : "New Perfume / Attar"}</h3>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div>
                ...
                <div className="grid grid-cols-5 gap-2">
                  {[0, 1, 2, 3, 4].map(i => (
                    <div key={i} className="aspect-square rounded-xl border-2 border-dashed border-gray-200 overflow-hidden relative group cursor-pointer bg-gray-50 hover:border-purple-400 transition-colors">
                      {previews[i] ? (
                        <>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="col-span-2">
                  ...
                  <input required type="text" value={f("name")} onChange={e => sf("name", e.target.value)} placeholder="e.g. Royal Oud Attar" className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-purple-400 focus:ring-2 focus:ring-purple-100" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-gray-600 uppercase tracking-wider mb-1">Category</label>
                  <input type="text" value={f("category")} onChange={e => sf("category", e.target.value)} placeholder="e.g. attar, eau-de-parfum" className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-purple-400 focus:ring-2 focus:ring-purple-100" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-gray-600 uppercase tracking-wider mb-1">Scent Family</label>
                  <input type="text" value={f("scentFamily")} onChange={e => sf("scentFamily", e.target.value)} placeholder="e.g. Woody, Oud, Floral" className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-purple-400 focus:ring-2 focus:ring-purple-100" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-gray-600 uppercase tracking-wider mb-1">Price (₹) *</label>
                  <input required type="number" min="1" value={f("price")} onChange={e => sf("price", e.target.value)} placeholder="0" className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-purple-400 focus:ring-2 focus:ring-purple-100" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-gray-600 uppercase tracking-wider mb-1">Weight (g)</label>
                  <input type="number" min="0" value={f("weightGrams")} onChange={e => sf("weightGrams", e.target.value)} placeholder="0" className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-purple-400 focus:ring-2 focus:ring-purple-100" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-gray-600 uppercase tracking-wider mb-1">Volume (ml)</label>
                  <input type="number" min="0" value={f("volumeMl")} onChange={e => sf("volumeMl", e.target.value)} placeholder="0" className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-purple-400 focus:ring-2 focus:ring-purple-100" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-gray-600 uppercase tracking-wider mb-1">Concentration</label>
                  <input type="text" value={f("concentration")} onChange={e => sf("concentration", e.target.value)} placeholder="e.g. Attar, EDP, EDT" className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-purple-400 focus:ring-2 focus:ring-purple-100" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-gray-600 uppercase tracking-wider mb-1">Longevity (hours)</label>
                  <input type="number" min="0" value={f("longevityHours")} onChange={e => sf("longevityHours", e.target.value)} placeholder="e.g. 12" className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-purple-400 focus:ring-2 focus:ring-purple-100" />
                </div>
                <div className="flex items-center pt-5 pl-2">
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" checked={formData.isAlcoholFree} onChange={e => sf("isAlcoholFree", e.target.checked)} className="w-4 h-4 text-purple-600 border-gray-300 rounded focus:ring-purple-500" />
                    <span className="text-xs font-bold text-gray-600 uppercase tracking-wider">Alcohol Free</span>
                  </label>
                </div>

                {/* Fragrance Notes */}
                <div className="col-span-2 grid grid-cols-3 gap-2 border-t border-b border-gray-100 py-3 my-1">
                  <div className="col-span-3">
                    <label className="block text-[10px] font-black text-gray-400 uppercase tracking-wider mb-1">Fragrance Notes</label>
                  </div>
                  <div>
                    <label className="block text-[10px] font-bold text-gray-500 uppercase mb-0.5">Top Note</label>
                    <input type="text" value={f("scentNotesTop")} onChange={e => sf("scentNotesTop", e.target.value)} placeholder="e.g. Rose, Bergamot" className="w-full px-2 py-1.5 border border-gray-200 rounded-lg text-xs focus:outline-none focus:border-purple-400" />
                  </div>
                  <div>
                    <label className="block text-[10px] font-bold text-gray-500 uppercase mb-0.5">Middle Note</label>
                    <input type="text" value={f("scentNotesMiddle")} onChange={e => sf("scentNotesMiddle", e.target.value)} placeholder="e.g. Saffron, Amber" className="w-full px-2 py-1.5 border border-gray-200 rounded-lg text-xs focus:outline-none focus:border-purple-400" />
                  </div>
                  <div>
                    <label className="block text-[10px] font-bold text-gray-500 uppercase mb-0.5">Base Note</label>
                    <input type="text" value={f("scentNotesBase")} onChange={e => sf("scentNotesBase", e.target.value)} placeholder="e.g. Oud, Sandalwood" className="w-full px-2 py-1.5 border border-gray-200 rounded-lg text-xs focus:outline-none focus:border-purple-400" />
                  </div>
                </div>

                <div className="col-span-2">
                  <label className="block text-xs font-bold text-gray-600 uppercase tracking-wider mb-1">Ingredients</label>
                  <textarea value={f("ingredients")} onChange={e => sf("ingredients", e.target.value)} placeholder="Pure sandalwood oil, natural essential oils..." rows={2} className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-purple-400 focus:ring-2 focus:ring-purple-100 resize-none" />
                </div>
              </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <button type="button" onClick={() => setBulkTiers(p => [...p, { minQty: "", pricePerPc: "" }])} className="text-xs font-bold text-purple-600 hover:underline">+ Add Tier</button>
                </div>
