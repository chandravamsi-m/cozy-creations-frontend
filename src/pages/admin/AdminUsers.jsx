    ...
    <div className="space-y-4 font-sans">
      ...
      <div className="flex flex-row items-center justify-between gap-2 lg:gap-6 py-2 px-1 border-y border-gray-100 bg-gray-50/10">
        ...
        <div className="flex-1 flex items-center bg-white border border-gray-200 rounded-xl shadow-sm focus-within:ring-2 focus-within:ring-blue-500/10 focus-within:border-blue-500 transition-all overflow-hidden h-10 lg:max-w-xl">
          ...
          />
          <div className="mr-1.5 p-1.5 lg:px-6 lg:py-1.5 bg-blue-50 text-blue-600 rounded-lg shrink-0 text-xs font-bold">
             Search
          </div>
        </div>
      ...
      <div className="">
        {loading ? (
           ...
           <div className="bg-white border border-gray-100 rounded-2xl p-24 text-center">
              ...
              <p className="text-xs font-bold text-gray-300">No users found</p>
           </div>
        ) : (
          <>
            {/* MOBILE CARD VIEW (< lg) */}
                ...
                <div key={u.id} className="bg-white border border-gray-100 rounded-2xl p-4 shadow-sm hover:shadow-md transition-all group">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        <p className="text-sm font-semibold text-gray-900 truncate tracking-tight">{u.displayName || "Anonymous"}</p>
                        <p className="text-[11px] text-gray-400 font-medium truncate">{u.email}</p>
                      </div>
                    </div>
                    <span className={`px-2 py-0.5 rounded-lg text-[10px] font-bold border shadow-sm ${u.role === 'admin' ? 'bg-blue-50 text-blue-600 border-blue-100' : 'bg-gray-50 text-gray-500 border-gray-100'}`}>
                      {u.role || 'user'}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center justify-between pt-3 border-t border-gray-50">
                    <div className="flex flex-col">
                      <span className="text-[11px] font-medium text-gray-400 mb-0.5">Last Active</span>
                      <span className="text-xs font-semibold text-gray-600">{formatRelativeTime(u.updatedAt)}</span>
                    </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-gray-50/50 border-b border-gray-100">
                    <th className="px-6 py-5 text-xs font-semibold text-gray-500 border-r border-gray-100/50">User Identity</th>
                    <th className="px-6 py-5 text-xs font-semibold text-gray-500 border-r border-gray-100/50">Account Level</th>
                    <th className="px-6 py-5 text-xs font-semibold text-gray-500 border-r border-gray-100/50">Last Active</th>
                    <th className="px-6 py-5 text-xs font-semibold text-gray-500 text-right">Actions</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-50">
                  {paginatedUsers.map(u => (
                    <tr key={u.id} className="hover:bg-blue-50/30 transition-all cursor-pointer group hover:scale-[1.002] duration-200">
                      ...
                      <td className="px-6 py-2.5 border-l border-gray-50/10">
                        <span className={`px-2.5 py-1 rounded-lg text-[10px] font-bold border shadow-sm ${u.role === 'admin' ? 'bg-blue-50 text-blue-600 border-blue-100' : 'bg-gray-50 text-gray-500 border-gray-100'}`}>{u.role || 'user'}</span>
                      </td>
                      <td className="px-6 py-2.5 border-l border-gray-50/10">
                        <div className="flex flex-col">
                          <span className="text-sm font-bold text-gray-600">{formatRelativeTime(u.updatedAt)}</span>
                          <span className="text-[11px] text-gray-400 font-medium">Active status</span>
                        </div>
                      </td>
                      <td className="px-6 py-2.5 text-right w-[120px]">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-[2px] animate-in fade-in duration-200" onClick={() => { setShowCreateModal(false); setShowP1(false); setShowP2(false); }}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleCreateUser} className="p-6 space-y-4 overflow-y-auto flex-1 custom-scrollbar">
              ...
              <div className="space-y-1.5">
                <label className="px-1 text-xs font-semibold text-gray-400 block">Password</label>
              ...
              <div className="space-y-1.5">
                <label className="px-1 text-xs font-semibold text-gray-400 block">Confirm Password</label>
                <div className="relative group">
              ...
              <div className="space-y-1.5">
                <label className="px-1 text-xs font-semibold text-gray-400 block">User Role</label>
                <div className="relative group">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-[2px] animate-in fade-in duration-200" onClick={() => { setShowEditModal(false); setShowEditP(false); }}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleSaveEdit} className="p-6 space-y-4 overflow-y-auto flex-1 custom-scrollbar">
              ...
              <div className="space-y-1.5">
                <label className="px-1 text-xs font-semibold text-gray-400 block">Reset Password</label>
                <div className="relative group">
                  <input type={showEditP ? "text" : "password"} value={editFormData.password} onChange={e => setEditFormData(p => ({ ...p, password: e.target.value }))} className="w-full border border-gray-200 px-4 py-2.5 rounded-xl text-sm font-medium focus:border-blue-500 transition-all outline-none bg-white pr-11" placeholder="Leave blank to keep current"/>
